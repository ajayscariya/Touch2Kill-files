...
public class DefaultMediaPicker implements VASTMediaPicker {
    ...
    private class AreaComparator implements Comparator<VASTMediaFile> {
        ...
        public int compare(VASTMediaFile obj1, VASTMediaFile obj2) {
            ...
            VASTLog.m3966v(DefaultMediaPicker.TAG, "AreaComparator: obj1:" + obj1Diff + " obj2:" + obj2Diff);
    ...
    private int prefilterMediaFiles(List<VASTMediaFile> mediaFiles) {
        ...
        while (iter.hasNext()) {
            ...
            if (TextUtils.isEmpty(mediaFile.getType())) {
                VASTLog.m3962d(TAG, "Validator error: mediaFile type empty");
                iter.remove();
            } else {
                BigInteger height = mediaFile.getHeight();
                if (height == null) {
                    VASTLog.m3962d(TAG, "Validator error: mediaFile height null");
                    iter.remove();
                } else {
                    int videoHeight = height.intValue();
                    if (videoHeight <= 0 || videoHeight >= maxPixels) {
                        VASTLog.m3962d(TAG, "Validator error: mediaFile height invalid: " + videoHeight);
                        iter.remove();
                    } else {
                        BigInteger width = mediaFile.getWidth();
                        if (width == null) {
                            VASTLog.m3962d(TAG, "Validator error: mediaFile width null");
                            iter.remove();
                        } else {
                            int videoWidth = width.intValue();
                            if (videoWidth <= 0 || videoWidth >= maxPixels) {
                                VASTLog.m3962d(TAG, "Validator error: mediaFile width invalid: " + videoWidth);
                                iter.remove();
                            } else if (TextUtils.isEmpty(mediaFile.getValue())) {
                                VASTLog.m3962d(TAG, "Validator error: mediaFile url empty");
                                iter.remove();
                            }
                        }
                    }
    ...
    private VASTMediaFile getBestMatch(List<VASTMediaFile> list) {
        VASTLog.m3962d(TAG, "getBestMatch");
