...
public class XmlTools {
    ...
    public static void logXmlDocument(Document doc) {
        VASTLog.m3962d(TAG, "logXmlDocument");
        try {
            Transformer transformer = TransformerFactory.newInstance().newTransformer();
            transformer.setOutputProperty(OutputKeys.OMIT_XML_DECLARATION, "no");
            transformer.setOutputProperty(OutputKeys.METHOD, Method.XML);
            transformer.setOutputProperty(OutputKeys.INDENT, "yes");
            transformer.setOutputProperty(OutputKeys.ENCODING, Defaults.Encoding);
            transformer.setOutputProperty("{http://xml.apache.org/xslt}indent-amount", "4");
            StringWriter sw = new StringWriter();
            transformer.transform(new DOMSource(doc), new StreamResult(sw));
            VASTLog.m3962d(TAG, sw.toString());
        } catch (Exception e) {
    ...
    public static String xmlDocumentToString(Document doc) {
        String xml = null;
        VASTLog.m3962d(TAG, "xmlDocumentToString");
        try {
            Transformer transformer = TransformerFactory.newInstance().newTransformer();
            transformer.setOutputProperty(OutputKeys.OMIT_XML_DECLARATION, "no");
            transformer.setOutputProperty(OutputKeys.METHOD, Method.XML);
    ...
    public static Document stringToDocument(String doc) {
        VASTLog.m3962d(TAG, "stringToDocument");
    ...
    public static String stringFromStream(InputStream inputStream) throws IOException {
        VASTLog.m3962d(TAG, "stringFromStream");
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        byte[] buffer = new byte[NodeFilter.SHOW_DOCUMENT_FRAGMENT];
        while (true) {
            int length = inputStream.read(buffer);
            if (length == -1) {
                return new String(baos.toByteArray(), Defaults.Encoding);
            }
    ...
    public static String getElementValue(Node node) {
        ...
        while (childIndex < childNodes.getLength()) {
            ...
            if (value.length() == 0) {
                ...
                VASTLog.m3966v(TAG, "getElementValue: " + value);
                return value;
            }
        }
        return value;
