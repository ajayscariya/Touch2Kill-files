...
public class VASTModelPostValidator {
    ...
    public static boolean validate(VASTModel model, VASTMediaPicker mediaPicker) {
        VASTLog.m3962d(TAG, Constants.DOM_VALIDATE);
            ...
            if (mediaPicker != null) {
                ...
                if (mediaFile != null) {
                    ...
                    if (!TextUtils.isEmpty(url)) {
                        ...
                        VASTLog.m3962d(TAG, "mediaPicker selected mediaFile with URL " + url);
                    }
                }
            } else {
                VASTLog.m3967w(TAG, "mediaPicker: We don't have a compatible media file to play.");
            }
            VASTLog.m3962d(TAG, "Validator returns: " + (isValid ? "valid" : "not valid (no media file)"));
            return isValid;
        }
        VASTLog.m3962d(TAG, "Validator returns: not valid (invalid model)");
        return false;
    }

    private static boolean validateModel(VASTModel model) {
        VASTLog.m3962d(TAG, "validateModel");
        ...
        VASTLog.m3962d(TAG, "Validator error: mediaFile list invalid");
        return false;
    }
}
