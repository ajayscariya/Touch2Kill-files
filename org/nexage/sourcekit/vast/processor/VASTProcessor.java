...
public final class VASTProcessor {
    ...
    private int processUri(InputStream is, int depth) {
        ...
        VASTLog.m3962d(TAG, "Doc is a wrapper. ");
        String nextUri = XmlTools.getElementValue(uriToNextDoc.item(0));
        VASTLog.m3962d(TAG, "Wrapper URL: " + nextUri);
        try {
    ...
    private Document createDoc(InputStream is) {
        VASTLog.m3962d(TAG, "About to create doc from InputStream");
        try {
            Document doc = DocumentBuilderFactory.newInstance().newDocumentBuilder().parse(is);
            doc.getDocumentElement().normalize();
            VASTLog.m3962d(TAG, "Doc successfully created.");
            return doc;
        } catch (Exception e) {
