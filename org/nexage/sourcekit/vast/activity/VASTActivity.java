...
public class VASTActivity extends Activity implements OnCompletionListener, OnErrorListener, OnPreparedListener, OnVideoSizeChangedListener, Callback {
    ...
    class C10935 extends TimerTask {
        ...
        class C10921 implements Runnable {
            ...
            public void run() {
                VASTLog.m3962d(VASTActivity.TAG, "hiding buttons");
        ...
        public void run() {
            VASTActivity.this.mHandler.post(new C10921());
        }
    }
    ...
    class C10946 extends TimerTask {
        ...
        public void run() {
            try {
                ...
                VASTLog.m3967w(VASTActivity.TAG, "mediaPlayer.getCurrentPosition exception: " + e.getMessage());
                cancel();
            }
        }
    }
    ...
    class C10957 extends TimerTask {
        int maxAmountInList;
            ...
            if (VASTActivity.this.mMediaPlayer != null) {
                if (VASTActivity.this.mVideoProgressTracker.size() == this.maxAmountInList) {
                    ...
                    if (lastPosition > firstPosition) {
                        VASTLog.m3966v(VASTActivity.TAG, "video progressing (position:" + lastPosition + ")");
                        VASTActivity.this.mVideoProgressTracker.removeFirst();
                    } else {
                        VASTLog.m3963e(VASTActivity.TAG, "detected video hang");
    ...
    protected void onCreate(Bundle savedInstanceState) {
        VASTLog.m3962d(TAG, "in onCreate");
        super.onCreate(savedInstanceState);
        int currentOrientation = getResources().getConfiguration().orientation;
        VASTLog.m3962d(TAG, "currentOrientation:" + currentOrientation);
        if (currentOrientation != 2) {
            VASTLog.m3962d(TAG, "Orientation is not landscape.....forcing landscape");
            setRequestedOrientation(0);
            return;
        }
        VASTLog.m3962d(TAG, "orientation is landscape");
        this.mVastModel = (VASTModel) getIntent().getSerializableExtra("com.nexage.android.vast.player.vastModel");
        if (this.mVastModel == null) {
            VASTLog.m3963e(TAG, "vastModel is null. Stopping activity.");
            finishVAST();
            return;
        }
        hideTitleStatusBars();
    ...
    protected void onStart() {
        VASTLog.m3962d(TAG, "entered onStart --(life cycle event)");
        super.onStart();
    }

    protected void onResume() {
        VASTLog.m3962d(TAG, "entered on onResume --(life cycle event)");
        super.onResume();
    }

    protected void onStop() {
        VASTLog.m3962d(TAG, "entered on onStop --(life cycle event)");
        super.onStop();
    }

    protected void onRestart() {
        VASTLog.m3962d(TAG, "entered on onRestart --(life cycle event)");
        super.onRestart();
        createMediaPlayer();
    }

    protected void onPause() {
        VASTLog.m3962d(TAG, "entered on onPause --(life cycle event)");
        super.onPause();
    ...
    protected void onDestroy() {
        VASTLog.m3962d(TAG, "entered on onDestroy --(life cycle event)");
        super.onDestroy();
    }

    public void onSaveInstanceState(Bundle savedInstanceState) {
        VASTLog.m3962d(TAG, "entered onSaveInstanceState ");
        super.onSaveInstanceState(savedInstanceState);
    }

    public void onRestoreInstanceState(Bundle savedInstanceState) {
        VASTLog.m3962d(TAG, "in onRestoreInstanceState");
    ...
    private void createOverlay(LayoutParams params) {
        ...
        this.mOverlay.setOnClickListener(new C10881());
    ...
    private void createInfoButton(int size) {
        ...
        if (clickThroughUrl != null && clickThroughUrl.length() > 0) {
            ...
            this.mInfoButton.setOnClickListener(new C10892());
    ...
    private void createCloseButton(int size) {
        ...
        this.mCloseButton.setOnClickListener(new C10903());
    ...
    private void createPlayPauseButton(int size) {
        ...
        this.mPlayPauseButton.setOnClickListener(new C10914());
        this.mButtonPanel.addView(this.mPlayPauseButton);
    }

    private void infoClicked() {
        VASTLog.m3962d(TAG, "entered infoClicked:");
    ...
    private void activateButtons(boolean active) {
        VASTLog.m3962d(TAG, "entered activateButtons:");
        if (active) {
    ...
    private void processClickThroughEvent() {
        VASTLog.m3962d(TAG, "entered processClickThroughEvent:");
        if (VASTPlayer.listener != null) {
            VASTPlayer.listener.vastClick();
        }
        String clickThroughUrl = this.mVastModel.getVideoClicks().getClickThrough();
        VASTLog.m3962d(TAG, "clickThrough url: " + clickThroughUrl);
        ...
        try {
            ...
            if (getPackageManager().resolveActivity(intent, 32) == null) {
                VASTLog.m3963e(TAG, "Clickthrough error occured, uri unresolvable");
            ...
            VASTLog.m3964e(TAG, e.getMessage(), e);
        }
    }

    private void closeClicked() {
        VASTLog.m3962d(TAG, "entered closeClicked()");
        cleanActivityUp();
        if (!this.mIsPlayBackError) {
            processEvent(TRACKING_EVENTS_TYPE.close);
        }
        finishVAST();
        VASTLog.m3962d(TAG, "leaving closeClicked()");
    }

    private void playPauseButtonClicked() {
        VASTLog.m3962d(TAG, "entered playPauseClicked");
        if (this.mMediaPlayer == null) {
            VASTLog.m3963e(TAG, "mMediaPlayer is null when playPauseButton was clicked");
            return;
        }
        boolean isPlaying = this.mMediaPlayer.isPlaying();
        VASTLog.m3962d(TAG, "isPlaying:" + isPlaying);
        if (isPlaying) {
            processPauseSteps();
    ...
    public void onBackPressed() {
        VASTLog.m3962d(TAG, "entered onBackPressed");
        closeClicked();
    }

    public void surfaceCreated(SurfaceHolder holder) {
        VASTLog.m3962d(TAG, "surfaceCreated -- (SurfaceHolder callback)");
        try {
            if (this.mMediaPlayer == null) {
                createMediaPlayer();
            }
            ...
            VASTLog.m3962d(TAG, "URL for media file:" + url);
    ...
    public void surfaceChanged(SurfaceHolder surfaceHolder, int arg1, int arg2, int arg3) {
        VASTLog.m3962d(TAG, "entered surfaceChanged -- (SurfaceHolder callback)");
    }

    public void surfaceDestroyed(SurfaceHolder surfaceHolder) {
        VASTLog.m3962d(TAG, "entered surfaceDestroyed -- (SurfaceHolder callback)");
        cleanUpMediaPlayer();
    }

    public void onVideoSizeChanged(MediaPlayer mp, int width, int height) {
        VASTLog.m3962d(TAG, "entered onVideoSizeChanged -- (MediaPlayer callback)");
        this.mVideoWidth = width;
        this.mVideoHeight = height;
        VASTLog.m3962d(TAG, "video size: " + this.mVideoWidth + "x" + this.mVideoHeight);
    }

    public void onPrepared(MediaPlayer mp) {
        VASTLog.m3962d(TAG, "entered onPrepared called --(MediaPlayer callback) ....about to play");
        calculateAspectRatio();
        this.mMediaPlayer.start();
        hideProgressBar();
        if (this.mIsVideoPaused) {
            VASTLog.m3962d(TAG, "pausing video");
            this.mMediaPlayer.pause();
        } else {
            startVideoProgressTimer();
        }
        VASTLog.m3962d(TAG, "current location in video:" + this.mCurrentVideoPosition);
        if (this.mCurrentVideoPosition > 0) {
            VASTLog.m3962d(TAG, "seeking to location:" + this.mCurrentVideoPosition);
    ...
    private void calculateAspectRatio() {
        VASTLog.m3962d(TAG, "entered calculateAspectRatio");
        if (this.mVideoWidth == 0 || this.mVideoHeight == 0) {
            VASTLog.m3967w(TAG, "mVideoWidth or mVideoHeight is 0, skipping calculateAspectRatio");
            return;
        }
        VASTLog.m3962d(TAG, "calculating aspect ratio");
        ...
        VASTLog.m3962d(TAG, " screen size: " + this.mScreenWidth + "x" + this.mScreenHeight);
        VASTLog.m3962d(TAG, " video size:  " + this.mVideoWidth + "x" + this.mVideoHeight);
        VASTLog.m3962d(TAG, " widthRatio:   " + widthRatio);
        VASTLog.m3962d(TAG, " heightRatio:   " + heightRatio);
        VASTLog.m3962d(TAG, "surface size: " + surfaceWidth + "x" + surfaceHeight);
    }
    ...
    private void cleanUpMediaPlayer() {
        VASTLog.m3962d(TAG, "entered cleanUpMediaPlayer ");
        if (this.mMediaPlayer != null) {
            if (this.mMediaPlayer.isPlaying()) {
    ...
    public boolean onError(MediaPlayer mp, int what, int extra) {
        VASTLog.m3963e(TAG, "entered onError -- (MediaPlayer callback)");
        this.mIsPlayBackError = true;
        VASTLog.m3963e(TAG, "Shutting down Activity due to Media Player errors: WHAT:" + what + ": EXTRA:" + extra + ":");
        processErrorEvent();
        closeClicked();
        return true;
    }
    ...
    private void processErrorEvent() {
        VASTLog.m3962d(TAG, "entered processErrorEvent");
        fireUrls(this.mVastModel.getErrorUrl());
    }

    public void onCompletion(MediaPlayer mediaPlayer) {
        VASTLog.m3962d(TAG, "entered onCOMPLETION -- (MediaPlayer callback)");
        stopVideoProgressTimer();
        stopToolBarTimer();
        this.mButtonPanel.setVisibility(0);
        this.mPlayPauseButton.setImageDrawable(this.mPlayDrawable);
    ...
    private void processImpressions() {
        VASTLog.m3962d(TAG, "entered processImpressions");
        this.mIsProcessedImpressions = true;
        fireUrls(this.mVastModel.getImpressions());
    }

    private void fireUrls(List<String> urls) {
        VASTLog.m3962d(TAG, "entered fireUrls");
        if (urls != null) {
            for (String url : urls) {
                VASTLog.m3966v(TAG, "\tfiring url:" + url);
                HttpTools.httpGetURL(url);
            }
            return;
        }
        VASTLog.m3962d(TAG, "\turl list is null");
    }

    private void startToolBarTimer() {
        VASTLog.m3962d(TAG, "entered startToolBarTimer");
        if (this.mQuartile != 4) {
            if (this.mMediaPlayer != null && this.mMediaPlayer.isPlaying()) {
                stopToolBarTimer();
                this.mToolBarTimer = new Timer();
                this.mToolBarTimer.schedule(new C10935(), TOOLBAR_HIDE_DELAY);
                this.mButtonPanel.setVisibility(0);
            }
            if (this.mIsVideoPaused) {
                activateButtons(true);
    ...
    private void stopToolBarTimer() {
        VASTLog.m3962d(TAG, "entered stopToolBarTimer");
    ...
    private void startQuartileTimer() {
        VASTLog.m3962d(TAG, "entered startQuartileTimer");
        stopQuartileTimer();
        if (this.mIsCompleted) {
            VASTLog.m3962d(TAG, "ending quartileTimer becuase the video has been replayed");
            return;
        }
        int videoDuration = this.mMediaPlayer.getDuration();
        this.mTrackingEventTimer = new Timer();
        this.mTrackingEventTimer.scheduleAtFixedRate(new C10946(videoDuration), 0, VIDEO_PROGRESS_TIMER_INTERVAL);
    }
    ...
    private void startVideoProgressTimer() {
        VASTLog.m3962d(TAG, "entered startVideoProgressTimer");
        this.mStartVideoProgressTimer = new Timer();
        this.mVideoProgressTracker = new LinkedList();
        this.mStartVideoProgressTimer.schedule(new C10957(), 0, VIDEO_PROGRESS_TIMER_INTERVAL);
    }

    private void stopVideoProgressTimer() {
        VASTLog.m3962d(TAG, "entered stopVideoProgressTimer");
    ...
    private void processEvent(TRACKING_EVENTS_TYPE eventName) {
        VASTLog.m3965i(TAG, "entered Processing Event: " + eventName);
