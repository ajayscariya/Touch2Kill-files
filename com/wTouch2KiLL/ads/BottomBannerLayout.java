...
import com.wTouch2KiLL.ads.behavior.bannerBehaviors.BannerLayoutBehavior;
import com.wTouch2KiLL.javascriptinterface.BannerJavascriptInterface;
...
public class BottomBannerLayout implements BehaviorAcceptor {
    ...
    public void init(Activity activity, AdsLoader adsLoader) {
        ...
        this._bannerBrowser = (BrowserWebView) activity.findViewById(C0866R.id.banner_webkit);
        ...
        if (this._activity instanceof MainNavigationActivity) {
            View tabtagsPanel = this._activity.findViewById(C0866R.id.tabtags_panel);
            View tabContentsPanel = this._activity.findViewById(C0866R.id.tabcontents_panel);
    ...
    public void applyDefaultSettings() {
        ...
        this._width = this._activity.getResources().getInteger(C0866R.integer.bannerWidth);
        this._height = this._activity.getResources().getInteger(C0866R.integer.bannerHeight);
        _applySize();
    ...
    public void setPosition(BannerPosition position) {
        if (position == BannerPosition.BOTTOM) {
            View bannerPanel = this._activity.findViewById(C0866R.id.banner_panel);
            ViewGroup contentFrame = (ViewGroup) this._activity.findViewById(C0866R.id.contentFrame);
    ...
    protected void _hideViewsForFullscreen() {
        if (this._activity instanceof MainNavigationActivity) {
            ...
            this._defaultTagsPanelVisibility = tabtagsPanel.getVisibility();
            this._defaultTabContentsVisibility = tabContentsPanel.getVisibility();
            this._activity.findViewById(C0866R.id.tabtags_panel).setVisibility(8);
    ...
    public void hideBanner() {
        ((TransparentPanel) this._activity.findViewById(C0866R.id.banner_panel)).setVisibility(8);
        this._bannerBrowser.setWebViewClient(null);
        _showViewsForFullscreen();
    }

    public void showBanner() {
        ((TransparentPanel) this._activity.findViewById(C0866R.id.banner_panel)).setVisibility(0);
        _showViewsForFullscreen();
    }

    protected void _showViewsForFullscreen() {
        if (this._activity instanceof MainNavigationActivity) {
            this._activity.findViewById(C0866R.id.tabtags_panel).setVisibility(this._defaultTagsPanelVisibility);
            this._activity.findViewById(C0866R.id.tabcontents_panel).setVisibility(this._defaultTabContentsVisibility);
        }
    }

    public void switchToHtmlAd() {
        FrameLayout layout = (FrameLayout) this._activity.findViewById(C0866R.id.ad_container);
        View adMobView = this._activity.findViewById(AD_MOB_VIEW_ID);
        if (adMobView != null) {
            layout.removeView(adMobView);
        }
        this._activity.findViewById(C0866R.id.banner_webkit).setVisibility(0);
    }

    public void switchToAdMobAd(AdMobParameters adMobParameters) {
        this._activity.findViewById(C0866R.id.banner_webkit).setVisibility(8);
