...
public class TextInputLayout extends LinearLayout {
    ...
    class C11194 implements AnimatorUpdateListener {
        C11194() {
        }
    ...
    class C15012 extends ViewPropertyAnimatorListenerAdapter {
        C15012() {
        }
    ...
    class C15023 extends ViewPropertyAnimatorListenerAdapter {
        final /* synthetic */ CharSequence val$error;

        C15023(CharSequence charSequence) {
    ...
    public void setHint(@Nullable CharSequence hint) {
        if (this.mHintEnabled) {
            ...
            sendAccessibilityEvent(XMLEntityManager.DEFAULT_BUFFER_SIZE);
        }
    }
        ...
        if (!TextUtils.equals(this.mError, error)) {
            ...
            if (this.mErrorShown) {
                ...
                if (animate) {
                    ...
                    ViewCompat.animate(this.mErrorView).alpha(DefaultRetryPolicy.DEFAULT_BACKOFF_MULT).setDuration(200).setInterpolator(AnimationUtils.LINEAR_OUT_SLOW_IN_INTERPOLATOR).setListener(new C15012()).start();
                }
            } else if (this.mErrorView.getVisibility() == 0) {
                if (animate) {
                    ViewCompat.animate(this.mErrorView).alpha(0.0f).setDuration(200).setInterpolator(AnimationUtils.FAST_OUT_LINEAR_IN_INTERPOLATOR).setListener(new C15023(error)).start();
                } else {
                    this.mErrorView.setVisibility(4);
                }
            }
    ...
    private void animateToExpansionFraction(float target) {
        if (this.mCollapsingTextHelper.getExpansionFraction() != target) {
            if (this.mAnimator == null) {
                ...
                this.mAnimator.setUpdateListener(new C11194());
            }
