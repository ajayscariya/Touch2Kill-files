...
public class XMLEntityManager implements XMLComponent, XMLEntityResolver {
    ...
    class C10371 implements PrivilegedAction {
        C10371() {
        }
    ...
    private static class EncodingInfo {
        ...
        static {
            UTF_8 = new EncodingInfo(Defaults.Encoding, null, XMLEntityManager.DEBUG_RESOLVER);
            UTF_8_WITH_BOM = new EncodingInfo(Defaults.Encoding, null, true);
            UTF_16_BIG_ENDIAN = new EncodingInfo("UTF-16", Boolean.TRUE, XMLEntityManager.DEBUG_RESOLVER);
            UTF_16_BIG_ENDIAN_WITH_BOM = new EncodingInfo("UTF-16", Boolean.TRUE, true);
            UTF_16_LITTLE_ENDIAN = new EncodingInfo("UTF-16", Boolean.FALSE, XMLEntityManager.DEBUG_RESOLVER);
    ...
    public class ScannedEntity extends Entity {
        ...
        public ScannedEntity(String name, XMLResourceIdentifier entityLocation, InputStream stream, Reader reader, byte[] byteBuffer, String encoding, boolean literal, boolean mayReadChunks, boolean isExternal) {
            ...
            this.xmlVersion = BuildConfig.VERSION_NAME;
            this.ch = null;
            this.entityLocation = entityLocation;
            this.stream = stream;
            this.reader = reader;
    ...
    static {
        ...
        GET_USER_DIR_SYSTEM_PROPERTY = new C10371();
    }
    ...
    public String setupCurrentEntity(String name, XMLInputSource xmlInputSource, boolean literal, boolean isExternal) throws IOException, XNIException {
        ...
        if (reader == null) {
            ...
            if (encoding == null) {
                ...
                if (encoding.equals(Defaults.Encoding)) {
                    int[] b3 = new int[3];
                    count = 0;
                    while (count < 3) {
                    ...
                    reader = createReader(stream, Defaults.Encoding, null);
    ...
    private static synchronized URI getUserDir() throws MalformedURIException {
        ...
        synchronized (XMLEntityManager.class) {
            String userDir = XMLConstants.NULL_NS_URI;
            try {
                userDir = (String) AccessController.doPrivileged(GET_USER_DIR_SYSTEM_PROPERTY);
            } catch (SecurityException e) {
            }
            if (userDir.length() == 0) {
                uri = new URI("file", XMLConstants.NULL_NS_URI, XMLConstants.NULL_NS_URI, null, null);
                ...
                if (i < len) {
                    try {
                        for (byte b : userDir.substring(i).getBytes(Defaults.Encoding)) {
                            if (b < null) {
                        ...
                        uri = new URI("file", XMLConstants.NULL_NS_URI, userDir, null, null);
                    }
                }
                if (!userDir.endsWith("/")) {
                    buffer.append('/');
                }
                gUserDirURI = new URI("file", XMLConstants.NULL_NS_URI, buffer.toString(), null, null);
                uri = gUserDirURI;
            } else {
                uri = gUserDirURI;
            }
                    ...
                    if (!(baseSystemId.length() == 0 || baseSystemId.equals(systemId))) {
                        try {
                            ...
                            if (baseSystemId.indexOf(58) != -1) {
                                base = new URI("file", XMLConstants.NULL_NS_URI, fixURI(baseSystemId).trim(), null, null);
                            } else {
    ...
    protected Reader createReader(InputStream inputStream, String encoding, Boolean isBigEndian) throws IOException {
        if (encoding == Defaults.Encoding || encoding == null) {
