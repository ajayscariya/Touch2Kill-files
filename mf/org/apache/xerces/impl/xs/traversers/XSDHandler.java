    ...
    private Element getSchemaDocument1(boolean mustResolve, boolean hasInput, XMLInputSource schemaSource, Element referElement, IOException ioe) {
        ...
        if (mustResolve) {
            if (hasInput) {
                ...
                objArr2[0] = schemaSource == null ? XMLConstants.NULL_NS_URI : schemaSource.getSystemId();
        ...
        switch (componentType) {
            case ATTRIBUTE_TYPE /*1*/:
                if (((XSAttributeDecl) component).getScope() == (short) 1) {
                    ...
                    if (sg.getGlobalAttributeDecl(name, XMLConstants.NULL_NS_URI) == null) {
                        sg.addGlobalAttributeDecl((XSAttributeDecl) component, XMLConstants.NULL_NS_URI);
                    }
                }
            case INIT_KEYREF_STACK /*2*/:
                if (((XSElementDecl) component).getScope() == (short) 1) {
                    ...
                    if (sg.getGlobalElementDecl(name, XMLConstants.NULL_NS_URI) == null) {
                        sg.addGlobalElementDecl((XSElementDecl) component, XMLConstants.NULL_NS_URI);
                    }
                }
            case ELEMENT_TYPE /*3*/:
                if (!((XSTypeDefinition) component).getAnonymous()) {
                    ...
                    if (sg.getGlobalTypeDecl(name, XMLConstants.NULL_NS_URI) == null) {
                        sg.addGlobalTypeDecl((XSTypeDefinition) component, XMLConstants.NULL_NS_URI);
                    }
                }
            case IDENTITYCONSTRAINT_TYPE /*5*/:
                if (sg.getGlobalAttributeDecl(name) == null) {
                ...
                if (sg.getGlobalAttributeDecl(name, XMLConstants.NULL_NS_URI) == null) {
                    sg.addGlobalAttributeGroupDecl((XSAttributeGroupDecl) component, XMLConstants.NULL_NS_URI);
                }
            case NOTATION_TYPE /*6*/:
                if (sg.getGlobalGroupDecl(name) == null) {
                    sg.addGlobalGroupDecl((XSGroupDecl) component);
                }
                if (sg.getGlobalGroupDecl(name, XMLConstants.NULL_NS_URI) == null) {
                    sg.addGlobalGroupDecl((XSGroupDecl) component, XMLConstants.NULL_NS_URI);
                }
            case ConnectionResult.LICENSE_CHECK_FAILED /*11*/:
                if (sg.getGlobalNotationDecl(name) == null) {
                    sg.addGlobalNotationDecl((XSNotationDecl) component);
                }
                if (sg.getGlobalNotationDecl(name, XMLConstants.NULL_NS_URI) == null) {
                    sg.addGlobalNotationDecl((XSNotationDecl) component, XMLConstants.NULL_NS_URI);
                }
            default:
        }
    }
        ...
        if (componentType.equals(SchemaSymbols.ELT_SIMPLETYPE)) {
            ...
            if (grandKidName.equals(SchemaSymbols.ELT_RESTRICTION)) {
                ...
                if (derivedBase == null || derivedBase.uri != currSchema.fTargetNamespace || !derivedBase.localpart.equals(oldName)) {
                    ...
                    objArr[ATTRIBUTE_TYPE] = new StringBuilder(String.valueOf(currSchema.fTargetNamespace == null ? XMLConstants.NULL_NS_URI : currSchema.fTargetNamespace)).append(",").append(oldName).toString();
                    reportSchemaError(str, objArr, child);
