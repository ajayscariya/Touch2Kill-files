...
public class XSDFACM implements XSCMValidator {
    ...
    private void buildDFA(CMNode syntaxTree) {
        ...
        this.fLeafCount = i + 1;
        this.fHeadNode = new XSCMBinOp(C0866R.styleable.Theme_checkboxStyle, syntaxTree, new XSCMLeaf(1, null, -1, i));
    ...
    private void calcFollowList(CMNode nodeCur) {
        if (nodeCur.type() == DownloadsListActivity.DOWNLOAD_COMPLETE) {
            calcFollowList(((XSCMBinOp) nodeCur).getLeft());
            calcFollowList(((XSCMBinOp) nodeCur).getRight());
        } else if (nodeCur.type() == C0866R.styleable.Theme_checkboxStyle) {
            calcFollowList(((XSCMBinOp) nodeCur).getLeft());
            calcFollowList(((XSCMBinOp) nodeCur).getRight());
    ...
    private void postTreeBuildInit(CMNode nodeCur) throws RuntimeException {
        ...
        } else if (nodeCur.type() == DownloadsListActivity.DOWNLOAD_COMPLETE || nodeCur.type() == C0866R.styleable.Theme_checkboxStyle) {
